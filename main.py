...
# Helper to retrieve examples from the feedback database.
def get_examples(source: str, limit: int = 3):
    """
    Fetch the most recent correction examples for a given source (e.g. smood, uber, smartbox) from the
    feedback database. Returns a list of tuples (model_csv, correct_csv).

    These examples are appended to the system prompt so the language model can learn from prior
    mistakes. Limit controls how many examples are returned.
    """
    conn = db()
    cur = conn.execute(
...
async def parse_pdf(file: UploadFile = File(...)):
    ...
    # Compute a hash of the PDF for potential future use (e.g. identifying duplicates)
    pdf_hash = sha256_bytes(pdf_bytes)
    # Determine the source platform (smood, uber, smartbox) based on filename or content.
    # For now we set it to "unknown"; clients can override via feedback API.
    source = "unknown"
    # Retrieve up to 3 recent examples for this source from the feedback database
    ...
    # Build a system prompt that includes few-shot examples to guide the model
    prompt_system = SYSTEM_PROMPT
    if few_shot:
        prompt_system += "\n\nUSA QUESTI ESEMPI COME REGOLA:\n" + few_shot
    # Write the PDF to a temporary file before uploading to OpenAI
    ...
    try:
        ...
        # Build the request payload for the assistant
        payload = [
...
async def feedback(payload: FeedbackPayload):
    """
    Endpoint to accept corrections from the client. Clients send the CSV generated by the model (model_csv)
    and the correct CSV (correct_csv) along with optional metadata (source, invoice_id, pdf_sha256).

    The feedback is stored in a SQLite database. Future calls to /parse will include the most recent
    examples for the corresponding source to improve accuracy via few-shot learning.
    """
    # Ensure the correct CSV is not empty
